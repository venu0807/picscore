...
const PLANS = {
  pro_monthly: {
    india: (userId: string) => ({ amount: 29900, currency: 'INR', planId: process.env.RAZORPAY_PLAN_PICSCORE_PRO, notes: { userId }, receipt: `pro_${userId}_${Date.now()}` }),
    global: { amount: 499, currency: 'USD', priceId: process.env.STRIPE_PRICE_PICSCORE_PRO },
  },
  lifetime: {
    india: (userId: string) => ({ amount: 299900, currency: 'INR', notes: { userId, tier: 'lifetime' }, receipt: `lifetime_${userId}_${Date.now()}` }),
    global: { amount: 2900, currency: 'USD', priceId: process.env.STRIPE_PRICE_PICSCORE_LIFETIME },
  },
};
...
export async function POST(req: NextRequest) {
  ...
  if (market === 'india') {
    ...
    if (!opts.planId) {
      ...
      return NextResponse.json({ error: 'Razorpay plan not configured. Set RAZORPAY_PLAN_PICSCORE_PRO' }, { status: 500 });
    }
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    success_url: `${origin}/score?checkout=success`,
